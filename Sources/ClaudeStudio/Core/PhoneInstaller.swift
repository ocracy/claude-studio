...
enum PhoneInstaller {
    ...
    static func writeRunner(machine: String) -> Bool {
        ...
        # A UTF-8 locale, for the same reason the PATH is embedded: launchd hands
        # an agent almost no environment. In the C locale tmux replaces every byte
        # it thinks unprintable in its format output with `_` — the TAB the bridge
        # splits fields on included — and every session then reads as "not
        # running" on the phone. `ttyd` and the terminals it attaches inherit this
        # too, so a Turkish session name arrives intact.
        export LANG=\(Shell.quoted(ProcessInfo.processInfo.environment["LANG"] ?? "en_US.UTF-8"))
